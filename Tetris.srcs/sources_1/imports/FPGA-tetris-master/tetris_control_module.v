module tetris_control_module
  ( clk, rst_n, move_right, acc,move_left, rotate_r, change,
  ...
  assign fall_down = (( count_down == T1S) | bottom ) & ( move_down_en ) ; //下落使能
  ...
  begin
    ...
    begin
      if(load_next_square_r)
      begin
        acc_dly <= T2S;
        acc_en<=0;
      end
      else if( ingame_sig & (|acc_dly))
        acc_dly <= acc_dly - 1'b1;
      else if( ingame_sig & (~|acc_dly))
        acc_en<=1;
    end
  end 

  assign bottom = acc & acc_en;
  /**************************************************/
  //判断下落的方块是否能下移
  ...
  begin
    ...
    begin
      rotate_test[0]<=moving_square_r[0];
      rotate_test[1]<=moving_square_r[1];
      rotate_test[2]<=moving_square_r[2];
      rotate_test[3]<=moving_square_r[3];
      rotate_test[4]<=moving_square_r[4];
      rotate_test[5]<=moving_square_r[5];
      rotate_test[6]<=moving_square_r[6];
      rotate_test[7]<=moving_square_r[7];
      rotate_test[8]<=moving_square_r[8];
      rotate_test[9]<=moving_square_r[9];
      rotate_test[10]<=moving_square_r[10];
      rotate_test[11]<=moving_square_r[11];
      rotate_test[12]<=moving_square_r[12];
      rotate_test[13]<=moving_square_r[13];
      rotate_test[14]<=moving_square_r[14];
      rotate_test[15]<=moving_square_r[15];
    end
  end
  ...
  begin
    if( !rst_n )
      ...
      load_next_square_r <= ( count_down == T1S ) && ( ~move_down_en );  //下落使能为0，即下落方块不能下移，发生碰撞
  end
    ...
    begin
      case( next_square_type )
        3'b000:
          moving_square_r <= 16'b0000_0111_0010_0000;
        3'b001:
          moving_square_r <= 16'b0000_0110_0110_0000;
        3'b010:
          moving_square_r <= 16'b0000_0000_1111_0000;
        3'b011:
          moving_square_r <= 16'b0000_0011_0110_0000;
        3'b100:
          moving_square_r <= 16'b0000_0110_0011_0000;
        3'b101:
          moving_square_r <= 16'b0000_0111_0100_0000;
        3'b110:
          moving_square_r <= 16'b0000_1110_0010_0000;
        default:
          moving_square_r <= 16'b0000_0110_0110_0000;
      endcase
    end
